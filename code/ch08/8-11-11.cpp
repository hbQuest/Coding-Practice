...
Date timNgayKeTiep(Date ngay) {
    if (ngay.th == 12 && ngay.ng == 31) {
        ngay.n++;
        ngay.th = 1;
        ngay.ng = 1;
    } else if (ngay.ng == ngayTrongThang(ngay.th, ngay.n)) {
        ngay.ng = 1;
        ngay.th++;
    } else {
        ngay.ng++;
...
Date timNgayKeTiepKNgay(Date ngay, int k) {
    for (int i = 1; i <= k; i++) {
...
int main() {
    ...
    Date ngayKeTiep = timNgayKeTiepKNgay(ngay, k);
    xuat(ngayKeTiep);
    printf(" la ngay sau %d ke tiep cua ngay ", k);
    xuat(ngay);
    return 0;
}