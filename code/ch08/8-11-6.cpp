...
int tinhNgayKeTuNgay111(Date ngay) {
    long long tong = (long long)(ngay.n - 1) * 365; // Ép kiểu cho ngay.n-1 sau đó mới tính nhân với 365 để tránh bị tràn số
    for (int i = 1; i < ngay.n; i++) {
        if (checkNamNhuan(i)) {
            tong += 1;
        }
    }
    int ngayTrongNam = tinhSoThuTuNgayTrongNam(ngay);
    tong += ngayTrongNam;
    return tong;
}
...
int main() {
    ...
    int n = tinhNgayKeTuNgay111(ngay);
    printf("%d/%d/%d la ngay thu %d ke tu ngay 1/1/1", ngay.ng, ngay.th, ngay.n, n);
