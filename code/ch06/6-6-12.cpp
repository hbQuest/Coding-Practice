...
void sapXepAmTangDuongGiam0GiuNguyen(int a[][MAX],int dong,int cot) {
    int n = dong * cot;
    for (int i = 0; i < n-1; i++) {
        for (int j = i+1; j < n; j++) {
            if (a[i/cot][i%cot] < 0 && a[j/cot][j%cot] < 0) {
                if (a[i/cot][i%cot] > a[j/cot][j%cot]) {
                    hoanVi(a[i/cot][i%cot], a[j/cot][j%cot]);
                }
            } else if (a[i/cot][i%cot] > 0 && a[j/cot][j%cot] > 0) {
                if (a[i/cot][i%cot] < a[j/cot][j%cot]) {
                    hoanVi(a[i/cot][i%cot], a[j/cot][j%cot]);
                }
            }
...
int main() {
    ...
    printf("Ma tran sau sap xep so am tang dan, duong giam dam, 0 giu nguyen:\n");
